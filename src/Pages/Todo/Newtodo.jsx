...
export default function Newtodo() {
  ...
  //  Add Todo
  const addTodo = () => {
    input.trim() === "" ? alert("empty") : setTodo([...todos, input]);
    setInput("");
  };

  // Delete Todo
  const deleteTodo = (m) => {
    const filteredTodo = todos.filter((todo, i) => i !== m);
  ...
  const editTodo = (m) => {
    setUpdateId(m);
    setEdit(true);
    setUpdateInput(todos[m])
    // let editedTodo = todos.find((todo, index) => index === m);
    // setInput(editedTodo);
